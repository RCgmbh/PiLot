...
var Analogclock = {
	...
	outer_tick_pill_path : [
		[156, -2],
		[180, -2],
		[180, 2],
		[156, 2]
	],
	outer_tick_head_path : [
		[180, -3],
		[184, -3],
		[184, 3],
		[180, 3]
	],
	outer_tick_major_path : [
		[174, -0.5],
		[184, -0.5],
		[184, 0.5],
		[174, 0.5]
	],
	outer_tick_minor_path : [
		[180, -0.3],
		[184, -0.3],
		[184, 0.3],
		[180, 0.3]
	...
	drawClock: function(id, utcOffset) {
		...
		Analogclock.pie_path[3][1] = tickOpts.pieLineWidth - 1;
		Analogclock.pie_path[2][1] = tickOpts.pieLineWidth - 1;
		...
		this.drawTicks = function (opts) {
			...
			// inner pie slices
			if (opts.pie) {
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 0);
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 30);
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 60);
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 90);
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 120);
				this.drawPathBk(Analogclock.pie_path, opts.pieColor, 150);
			}

			// triangle tics

			if (opts.tri) {
				this.drawPathBk(Analogclock.tri_path, opts.triColor, 0);
				this.drawPathBk(Analogclock.tri_path, opts.triColor, 90);
				this.drawPathBk(Analogclock.tri_path, opts.triColor, 180);
				this.drawPathBk(Analogclock.tri_path, opts.triColor, -90);
			} // outer ticks

			if (opts.outerTickPill) {
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 30);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 60);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 120);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 150);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, -30);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, -60);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, -120);
				this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, -150);
				...
				{
					this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 0);
					this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 90);
					this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 180);
					this.drawPathBk(Analogclock.outer_tick_pill_path, opts.outerTickPillColor, 270);
				}
			}
			...
			for (i = 0; i < 360; i += 30) {
				// outer tick pills
				if (opts.outerTickHead) this.drawPathBk(Analogclock.outer_tick_head_path, opts.outerTickHeadColor, i);

				// inner tick pills
				if (opts.innerTickHead) this.drawPathBk(Analogclock.inner_tick_head_path, opts.innerTickHeadColor, i);
				...
				for (var j = 0; j < 25; j += 6) {
					...
					{
						// major outer ring
						if (opts.outerTickMajor) this.drawPathBk(Analogclock.outer_tick_major_path, opts.outerTickMajorColor, i + j);
					}

					if (j != 0 || !opts.innerTickHead) // if not drawing tick heads and inner ticks enabled fill them in too
					{
						// inner ring major 
						if (opts.innerTick) this.drawPathBk(Analogclock.inner_tick_path, opts.innerTickColor, i + j);
					}
					...
					if (opts.outerTickMinor) // fine ticks, back fill if majors are not drawn
					{
						...
						for (; k < 5; k += 1.5)
						this.drawPathBk(Analogclock.outer_tick_minor_path, opts.outerTickMinorColor, i + j + k);
					}
				}
			}
		...
		this.drawSecHand = function (ang, opts) {
			this.drawPathFr(Analogclock.sec_hand_path, opts.secHandColor, ang);
		...
		this.drawMinHand = function (ang, opts) {
			this.drawPathFr(Analogclock.min_hand_path, opts.minHandColor, ang);
		}

		this.drawHourHand = function (ang, opts) {
			this.drawPathFr(Analogclock.hour_hand_path, opts.hourHandColor, ang);
		}
